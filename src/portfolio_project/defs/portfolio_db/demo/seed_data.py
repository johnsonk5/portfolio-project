...
def _safe_extract_zip(zip_path: Path, extract_to: Path) -> int:
    ...
    with ZipFile(zip_path, "r") as zip_file:
        for member in zip_file.infolist():
            ...
            if (
                extract_to_resolved not in target_path.parents
                and target_path != extract_to_resolved
            ):
...
def _select_release_asset(release_payload: dict, asset_name: str) -> dict:
    ...
    for release_asset in assets:
        if str(release_asset.get("name", "")) == asset_name:
...
def _load_source_bytes(
    ...
    if demo_repo and demo_asset:
        ...
        release_asset = _select_release_asset(release_payload, demo_asset)
        asset_id = str(release_asset.get("id", ""))
        ...
        if zip_path.exists() and _marker_matches(marker_path, marker_data):
            return (
                zip_path.read_bytes(),
                f"github:{demo_repo}@{release_tag}/{demo_asset}",
                marker_data,
                False,
            )
    ...
    if local_path is not None and local_path.exists():
        return (
            ...
            f"local:{local_path}",
            {"mode": "local", "path": str(local_path)},
            True,
        )
    if url:
    ...
    raise ValueError(
        "No demo zip source configured. Set DEMO_DATA_REPO+DEMO_DATA_ASSET, "
        "DEMO_DATA_ZIP_LOCAL_PATH, or DEMO_DATA_ZIP_URL."
    )
